...
// parseBasicAuth extracts (username, password, ok) from an Authorization header value.
...
// MimirAuthMiddleware authenticates requests destined for the Mimir proxy.
// It accepts two credential types:
//   - Admin credentials (MIMIR_ADMIN_USER / MIMIR_ADMIN_PASSWORD env vars): grants
//     unrestricted access to all Mimir paths; sets "mimir_is_admin" = true in context.
//   - System credentials (DB-backed Basic Auth): grants access only to the per-tenant
//     alertmanager paths; sets "mimir_is_admin" = false and "system_id" in context.
func MimirAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		auth := c.GetHeader("Authorization")
		if auth == "" {
			c.Header("WWW-Authenticate", `Basic realm="Mimir"`)
		...
		// Check admin credentials first (constant-time comparison not strictly needed
		// because the admin password is never user-supplied during normal operation,
		// but we keep it simple and safe).
		adminUser := configuration.Config.MimirAdminUser
		adminPass := configuration.Config.MimirAdminPassword
		if adminUser != "" && adminPass != "" && user == adminUser && password == adminPass {
			c.Set("mimir_is_admin", true)
			logger.Debug().
				Str("client_ip", c.ClientIP()).
				Str("path", c.Request.URL.Path).
				Msg("Mimir admin authenticated")
			c.Next()
			return
		}

		// Fall back to system credentials (DB-backed).
		systemID, valid := validateSystemCredentials(c, user, password)
		if !valid {
			c.Header("WWW-Authenticate", `Basic realm="Mimir"`)
			c.JSON(http.StatusUnauthorized, response.Unauthorized("invalid credentials", nil))
			c.Abort()
			return
		}

		c.Set("mimir_is_admin", false)
		c.Set("system_id", systemID)
		c.Set("system_key", user)
		...
		logger.Debug().
			...
			Str("client_ip", c.ClientIP()).
			Str("path", c.Request.URL.Path).
			Msg("Mimir system authenticated")
