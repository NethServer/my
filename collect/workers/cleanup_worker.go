...
// Sized for postgres basic_256mb (work_mem ~1.6MB): the original single-shot
// query OOM-killed the DB while building ROW_NUMBER over 470k+ rows.
const inventoryCleanupBatchSize = 1000

// inventoryCleanupStmtTimeout caps each cleanup statement so a slow plan
// doesn't sit on a connection for minutes during a recovery cycle.
...
//
// Implementation: the previous single-shot query OOM-killed postgres on
// 2026-05-03 because it materialized ROW_NUMBER + two NOT IN(DISTINCT ON)
// scans over the full 1.8GB table at once. We now split the work into per-tier
// batched DELETEs with a precomputed edge set, so peak working memory per
...
// preserved: the first and last record per system. Backed by the
// (system_id, created_at) index, this is two index-friendly DISTINCT ON scans
// instead of two full table NOT IN subqueries on every batch.
