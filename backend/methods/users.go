...
func CreateUser(c *gin.Context) {
	...
	if len(request.UserRoleIDs) == 0 {
		c.JSON(http.StatusBadRequest, response.ValidationFailed("validation failed", []response.ValidationError{
			{Key: "user_role_ids", Message: "at least one role is required"},
		}))
		return
	}
...
func GetUsers(c *gin.Context) {
	...
	userOrgRole := strings.ToLower(user.OrgRole)
	accounts, totalCount, err := service.ListUsers(userOrgRole, user.OrganizationID, user.ID, page, pageSize, search, sortBy, sortDirection, organizationFilter, statuses, roleFilter)
	if err != nil {
		logger.Error().
			Err(err).
			Str("user_id", user.ID).
...
func UpdateUser(c *gin.Context) {
	...
	// Validate that roles are not being set to empty
