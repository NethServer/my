-- Migration: Convert distributors.active boolean to deleted_at timestamp
...
ALTER TABLE distributors ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE;
...
DROP INDEX IF EXISTS idx_distributors_logto_id;
DROP INDEX IF EXISTS idx_distributors_active;

-- Step 4: Create new indexes using deleted_at pattern
CREATE UNIQUE INDEX IF NOT EXISTS idx_distributors_logto_id 
ON distributors(logto_id) 
WHERE logto_id IS NOT NULL AND deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_distributors_deleted_at 
ON distributors(deleted_at);

-- Step 5: Drop the active column (after ensuring all data is migrated)
ALTER TABLE distributors DROP COLUMN IF EXISTS active;
