...
echo "Original SUPPORT_SERVICE_NAME=$SUPPORT_SERVICE_NAME"
...
if [ "$IS_PULL_REQUEST" = "true" ]; then
    ...
    echo "Adjusted SUPPORT_SERVICE_NAME=$SUPPORT_SERVICE_NAME"
...
envsubst '$PORT $BACKEND_SERVICE_NAME $COLLECT_SERVICE_NAME $SUPPORT_SERVICE_NAME $FRONTEND_SERVICE_NAME $RESOLVER $LOGTO_ENDPOINT' < /etc/nginx/nginx.conf > /tmp/nginx.conf
