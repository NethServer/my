...
Each system belongs to an organization. The collect service resolves the system's `organization_id` from its credentials and injects it as the `X-Scope-OrgID` header before forwarding to Mimir. This ensures metrics are fully isolated between organizations — each organization only sees its own data.
