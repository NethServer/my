...
const { state: orgsState } = useQuery(organizationsQuery)
...
watch(
  () => orgsState.value.data,
  (orgs) => {
    if (orgs?.length && !selectedOrgId.value) {
      const first = orgs.find((o) => o.type.toLowerCase() !== 'owner')
      if (first) selectedOrgId.value = first.logto_id
    }
  },
  { immediate: true },
)
    ...
    <template v-else>
      ...
      <div class="mb-6 max-w-sm">
        <NeCombobox
          ...
          :placeholder="
            orgsState.status === 'pending'
              ? $t('common.loading')
              : $t('organizations.choose_organization')
          "
          :disabled="orgsState.status === 'pending'"
      ...
      <template v-if="selectedPageTab === 'alerts'">
        ...
        <div class="mb-4 flex items-center justify-between gap-4">
          ...
          <div class="flex shrink-0 items-center gap-2">
            ...
            />
            <NeButton kind="tertiary" size="sm" @click="stateFilter = [...stateFilter]">
              <template #prefix>
