...
const importTypeOptions = computed<RadioOption[]>(() => [
  { id: 'skip', label: t('users.import_type_skip') },
  { id: 'update', label: t('users.import_type_overwrite') },
])
...
const {
  ...
  onSuccess(data) {
    ...
    setTimeout(() => {
      ...
      if (data.created > 0)
        resultParts.push(t('users.import_result_created', { count: data.created }))
      if (data.updated > 0)
        resultParts.push(t('users.import_result_updated', { count: data.updated }))
      if (data.skipped > 0)
        resultParts.push(t('users.import_result_skipped', { count: data.skipped }))
      if (data.failed > 0) resultParts.push(t('users.import_result_failed', { count: data.failed }))

      notificationsStore.createNotification({
        kind: 'success',
        title: t('users.users_imported'),
...
function onValidate() {
  ...
  if (!csvFile.value) {
    fileValidationError.value = t('users.import_no_file_selected')
    return
  }

  if (!csvFile.value.name.endsWith('.csv') && csvFile.value.type !== 'text/csv') {
    fileValidationError.value = t('users.import_file_must_be_csv')
    return
  }
...
function formatErrorParams(issue: ImportFieldError): string[] {
  if (issue.candidates) {
    const candidateLabels = issue.candidates.map((c) => t(`organizations.${c.type}`)).join(', ')
...
function errorSummaryText(row: ImportRow): string {
  ...
  const message = t(`users.import_error_${issue.field}_${issue.message}`, params)
  return t('users.import_row_and_message', { row_number: row.row_number, name, message })
}
</script>
...
<template>
  <NeModal
    ...
    :primary-label="
      step === 'upload'
        ? $t('users.import_validate')
        : $t('users.import_confirm', { count: importCount })
    "
    ...
    :secondary-label="step === 'preview' ? $t('common.back') : undefined"
  ...
  >
    ...
    <div v-if="step === 'upload'" class="space-y-6">
      <p class="text-sm text-gray-500 dark:text-gray-400">{{ $t('users.import_description') }}</p>
      <div class="space-y-1.5">
        <NeFileInput
          ref="fileInputRef"
          v-model="csvFile"
          :label="$t('users.import_file_label')"
        ...
        />
        <i18n-t
          keypath="users.import_to_get_started"
          tag="p"
      ...
      <NeInlineNotification
        v-if="validateError"
        kind="error"
        :title="$t('users.import_validation_failed')"
      >
        <template #description>
          <i18n-t keypath="users.import_validation_error_message" tag="p">
            <template #link>
              <button
                type="button"
                ...
                @click="downloadTemplate"
              >
                {{ $t('users.download_the_template_lc') }}
              </button>
            </template>
          </i18n-t>
        </template>
    ...
    <div v-else-if="step === 'preview' && validationResult" class="space-y-6">
      ...
      <NeRadioSelection
        ...
        :label="$t('users.import_existing_users_label')"
      >
        <template #tooltip>
          <NeTooltip>
            <template #content>
              {{ $t('users.import_existing_users_tooltip') }}
            </template>
          </NeTooltip>
        </template>
      </NeRadioSelection>
      ...
      <div class="text-sm text-gray-600 dark:text-gray-300">
        <NeFormItemLabel>{{ $t('users.import_summary') }}</NeFormItemLabel>
        <p>{{ $t('users.import_summary_detected', { count: validationResult.total_rows }) }}</p>
        <ul class="mt-1 ml-5 list-disc space-y-0.5">
          <li>{{ $t('users.import_summary_valid', { count: validationResult.valid_rows }) }}</li>
          <li
            v-if="validationResult.error_rows + validationResult.ambiguous_rows > 0"
            class="text-rose-700 dark:text-rose-500"
          >
            {{
              $t('users.import_summary_errors', {
          ...
          <li v-if="validationResult.warning_rows > 0" class="text-amber-700 dark:text-amber-500">
            {{
              existingUsersOption === 'skip'
                ? $t('users.import_summary_warnings_skip', { count: validationResult.warning_rows })
                : $t('users.import_summary_warnings_update', {
      ...
      <NeInlineNotification
        ...
        kind="error"
        :title="$t('users.import_rows_with_errors', { count: errorRows.length })"
      >
        <template #description>
      ...
      <NeInlineNotification
        ...
        :title="
          existingUsersOption === 'skip'
            ? $t('users.import_rows_with_warnings_skip', { count: warningRows.length })
            : $t('users.import_rows_with_warnings_update', { count: warningRows.length })
        "
      >
        <template #description>
          <ul class="ml-4 list-disc space-y-0.5">
      ...
      <NeInlineNotification
        v-if="confirmError"
        kind="error"
        :title="$t('users.import_confirm_failed')"
