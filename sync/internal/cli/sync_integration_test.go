...
func TestCheckLogtoInitialization(t *testing.T) {
	// Save original environment
	originalClientID := os.Getenv("LOGTO_BACKEND_APP_ID")
	defer func() {
		if originalClientID == "" {
			_ = os.Unsetenv("LOGTO_BACKEND_APP_ID")
		} else {
			_ = os.Setenv("LOGTO_BACKEND_APP_ID", originalClientID)
		}
	}()
	...
	t.Run("missing backend client ID env var", func(t *testing.T) {
		...
		backendClientID := os.Getenv("LOGTO_BACKEND_APP_ID")
	...
	t.Run("backend client ID set", func(t *testing.T) {
		_ = os.Setenv("LOGTO_BACKEND_APP_ID", "test-backend-client")
...
func TestSyncCommandValidation(t *testing.T) {
	...
	envVars := []string{"LOGTO_TENANT_ID", "LOGTO_BACKEND_APP_ID", "LOGTO_BACKEND_APP_SECRET"}
